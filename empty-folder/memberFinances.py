...
def memberSponsoredBills(bioGuideID):
    r = requests.get(
        f"{BASE_URL}/member/{bioGuideID}/sponsored-legislation",
        params={
            "api_key": API_KEY,
            "limit": 20,
...
def CongressMembersID(bioGuideID):
    ...
    return r.json()
def memberPicture(bioguideId):
    return CongressMembersID(bioguideId).get("member").get("depiction").get("imageUrl")
...
API_key="8C33A6HCJeNAHEqzoWKlSLtbOu7vkhWKU2PNXDqp"
base_url="https://api.open.fec.gov/v1"

#0.30(small-donor percentile)+0.25(100−PAC-dependence percentile)+0.20(100−donor-concentration percentile)+0.15(in-state donor percentile)+0.10(100−self/party-funding percentile)

def getFECId(candidate):
    r = requests.get(
    ...
    return r.json()['results'][0]['principal_committees'][0].get("committee_id")
def fundingNumbers(candidate):
    id=getFECId(candidate)
    r = requests.get(
        f"{base_url}/committee/{id}/totals",
        params={
            "q": candidate,
            "api_key": API_key,
            "limit": 20,
